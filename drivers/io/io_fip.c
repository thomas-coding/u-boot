...
static inline int compare_uuids(const fip_uuid_t *uuid1, const fip_uuid_t *uuid2)
{
	return memcmp(uuid1, uuid2, sizeof(fip_uuid_t));
}
...
static int free_dev_info(io_dev_info_t *dev_info)
{
	...
	if (result ==  0) {
		...
		memset(state, 0, sizeof(fip_dev_state_t));
		--fip_dev_count;
	}
			 ...
			 io_entity_t *entity)
{
	...
	static const fip_uuid_t uuid_null = { {0} }; /* Double braces for clang */
	size_t bytes_read;
	int found_file = 0;
	...
	if (current_fip_file.entry.offset_address != 0U) {
		printf("fip_file_open : Only one open file at a time.\n");
		return -ENFILE;
	}
	...
	result = io_open(backend_dev_handle, backend_image_spec,
			 &backend_handle);
	if (result != 0) {
		printf("Failed to open Firmware Image Package (%i)\n", result);
		result = -ENOENT;
	...
	if (result != 0) {
		printf("fip_file_open: failed to seek\n");
		result = -ENOENT;
			  ...
			  size_t *length_read)
{
	...
	if (result != 0) {
		printf("fip_file_read: failed to seek\n");
		result = -ENOENT;
	...
	if (result != 0) {
		...
		printf("Failed to read payload (%i)\n", result);
		result = -ENOENT;
		goto fip_file_read_close;
	} else {
...
static int fip_file_close(io_entity_t *entity)
{
	...
	if (current_fip_file.entry.offset_address != 0U) {
		...
		memset(&current_fip_file, 0, sizeof(current_fip_file));
	}
