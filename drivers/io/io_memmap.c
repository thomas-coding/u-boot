...
static int memmap_dev_open(const uintptr_t dev_spec,
			     ...
			     io_entity_t *entity)
{
	...
	if (current_memmap_file.in_use == 0) {
		...
		printf("A Memmap device is already active. Close first.\n");
	}
...
static int memmap_block_close(io_entity_t *entity)
{
	...
	entity->info = 0;
	//printf("wjp memmap_block_close \n");
	/* This would be a mem free() if we had malloc.*/
	//zeromem((void *)&current_memmap_file, sizeof(current_memmap_file));
	memset((void *)&current_memmap_file, 0, sizeof(current_memmap_file));

	//printf("wjp memmap_block_close in_use:%d\n", current_memmap_file.in_use);
	return 0;
}
