...
static const fip_uuid_t uuid_null;
static int verbose;
...
static image_desc_t *new_image_desc(const fip_uuid_t *uuid,
    const char *name, const char *cmdline_name)
{
	...
	memcpy(&desc->uuid, uuid, sizeof(fip_uuid_t));
...
static image_desc_t *lookup_image_desc_from_uuid(const fip_uuid_t *uuid)
{
	image_desc_t *desc;

	for (desc = image_desc_head; desc != NULL; desc = desc->next)
		if (memcmp(&desc->uuid, uuid, sizeof(fip_uuid_t)) == 0)
			return desc;
	return NULL;
}
...
static void fip_uuid_to_str(char *s, size_t len, const fip_uuid_t *u)
{
...
static void uuid_from_str(fip_uuid_t *u, const char *s)
{
	int n;
...
static int parse_fip(const char *filename, fip_toc_header_t *toc_header_out)
{
	...
	while ((char *)toc_entry + sizeof(*toc_entry) - 1 < bufend) {
		...
		if (memcmp(&toc_entry->uuid, &uuid_null, sizeof(fip_uuid_t)) == 0) {
			terminated = 1;
			break;
		}
		...
		if (desc == NULL) {
			...
			fip_uuid_to_str(name, sizeof(name), &toc_entry->uuid);
...
static image_t *read_image_from_file(const fip_uuid_t *uuid, const char *filename)
{
	struct BLD_PLAT_STAT st;
	image_t *image;
	FILE *fp;
...
static void parse_blob_opt(char *arg, fip_uuid_t *uuid, char *filename, size_t len)
{
	char *p;
...
static int create_cmd(int argc, char *argv[])
{
	...
	while (1) {
		...
		case 'b': {
			...
			fip_uuid_t uuid = uuid_null;
			image_desc_t *desc;

			parse_blob_opt(optarg, &uuid,
			    filename, sizeof(filename));

			if (memcmp(&uuid, &uuid_null, sizeof(fip_uuid_t)) == 0 ||
			    filename[0] == '\0')
				create_usage(EXIT_FAILURE);

			desc = lookup_image_desc_from_uuid(&uuid);
			if (desc == NULL) {
				fip_uuid_to_str(name, sizeof(name), &uuid);
