...
# IMG_CHECKSUM_BIN defines the default checksum image file corresponding to a
# BL stage
#   $(1) = BL stage
define IMG_CHECKSUM_BIN
    ${BUILD_PLAT}/$(1)_checksum.bin
endef
...
$(2): $(1) enctool
	$$(ECHO) "  ENC     $$<"
	$$(Q)$$(ENCTOOL) $$(ENC_ARGS) -i $$< -o $$@ -k $$(ENC_KEY) -n $$(ENC_NONCE) -a $$(DECRYPTION_SUPPORT) -f $${FW_ENC_STATUS}
endef

# CHECKSUM_FW invokes checksumtool to checksum firmware binary
#   $(1) = input firmware binary
#   $(2) = output checksum firmware binary
define CHECKSUM_FW
$(2): $(1) checksumtool
	$$(ECHO) "  CHECKSUM     $$<"
	$$(Q)$$(CHECKSUMTOOL) $$(CHECKSUM_ARGS) -s $$< -d $$@ -e $$(ENCRYPT_INFO)
endef
...
# FIP_ADD_CMD_OPT adds a new command line option to the cert_create invocation
#   $(1) = parameter filename
#   $(2) = fiptool command line option for the specified parameter
#   $(3) = FIP prefix (optional) (if FWU_, target is fwu_fip instead of fip)
define FIP_ADD_CMD_OPT
    $(3)FIP_ARGS += $(2) $(1)
endef
...
#   $(5) = Image checksum flag (optional) (0, 1)
# Example:
...
ifeq ($(4),1)
    ...
    ifeq ($(5),1)
        $(eval CHECKSUM_BIN := ${BUILD_PLAT}/$(1)_checksum.bin)
        $(call CHECKSUM_FW,$(ENC_BIN),$(CHECKSUM_BIN))
        $(call TOOL_ADD_IMG_PAYLOAD,$(1),$(value $(_V)),$(2),$(CHECKSUM_BIN),$(3),$(CHECKSUM_BIN))
    else
        $(call TOOL_ADD_IMG_PAYLOAD,$(1),$(value $(_V)),$(2),$(ENC_BIN),$(3),$(ENC_BIN))
    endif
else
    ifeq ($(5),1)
        $(eval CHECKSUM_BIN := ${BUILD_PLAT}/$(1)_checksum.bin)
        $(call CHECKSUM_FW,$(value $(_V)),$(CHECKSUM_BIN))
        $(call TOOL_ADD_IMG_PAYLOAD,$(1),$(value $(_V)),$(2),$(CHECKSUM_BIN),$(3),$(CHECKSUM_BIN))
    else
...
#   $(5) = BL checksum flag (optional) (0, 1)
define MAKE_BL
        ...
        $(eval CHECKSUM_BIN    := $(call IMG_CHECKSUM_BIN,$(1)))
...
ifeq ($(5),1)
$(call CHECKSUM_FW,$(ENC_BIN),$(CHECKSUM_BIN))
$(if $(2),$(call TOOL_ADD_IMG_PAYLOAD,$(1),$(BIN),--$(2),$(CHECKSUM_BIN),$(3), \
		$(CHECKSUM_BIN)))
else
...
ifeq ($(5),1)
$(call CHECKSUM_FW,$(BIN),$(CHECKSUM_BIN))
$(if $(2),$(call TOOL_ADD_IMG_PAYLOAD,$(1),$(CHECKSUM_BIN),--$(2),$(CHECKSUM_BIN),$(3)))
else
